...
class GroupModal extends Component {
  ...
  addGroup = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if(!err){
        ...
        // this logic is used to add directly to db 
        console.log(groupData); //adding consoles to show earlier group data
        var userDetails = [];
        var moodValue = [{updated_at: new Date().toISOString(), moodValue: ""}]
        values.users.forEach(function(obj){
          userDetails.push({ userName:obj.label, MoodRegistered: moodValue })
        })
        groupData.push({
        ...
        this.setState({loading: false});
        console.log(groupData); //adding console to show the added group
        var len = groupData.length;
        this.getGroupData(groupData[len-1]);
        this.showFormTrue(false)
      }
    });
  }
  ...
  fetchUser = (value) => {
    ...
    fetch('https://randomuser.me/api/?results=5')    //this is random api used to search for people
  ...
  getGroupData = (vData) => {
    this.props.getCallToBeInitialized(vData);
  }
